...
def test_login_with_wrong_credentials(test_client, app):
    response = test_client.post('/auth/login', data=dict(username='asdff', password='asdf'), follow_redirects=True)
...
def test_login_with_good_credentials(test_client, user):
    response = test_client.post('/auth/login', data=dict(username=user.username, password='asdf'), follow_redirects=True)
    assert response.status_code == 200
...
def test_protected_page_access_logged_in(logged_in_client):
